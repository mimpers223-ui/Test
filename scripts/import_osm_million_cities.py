#!/usr/bin/env python3
"""
Импорт АЗС городов-миллионников РФ из OpenStreetMap.

Данные пред-загружены локально в data/osm_million_cities.json
(Overpass API заблокирован с Render — нельзя загружать онлайн).
...
# === Конфиг ===
REGION_NAME = "Миллионники"
DATA_FILE = os.path.join(os.path.dirname(__file__), "..", "data", "osm_million_cities.json")
...
async def main() -> dict:
    logger.info(f"=== Импорт АЗС городов-миллионников из OSM (из файла) ===")
    ...
    by_city = Counter(s.get("city", "?") for s in stations)
    logger.info("По городам:")
